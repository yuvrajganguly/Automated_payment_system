...
EVENT_COLORS = {
    ...
    "RENT": "FCE4D6",
    "RENT_MISSED": "FF7F7F",
    "RENT_RECOVERED": "DAE8FC",
    "DUES_CARRY": "FFF2CC",
    "ADJUSTMENT": "E1D5E7",
    "DEDUCTION_SWITCH": "D5E8D4",
    "EV_SWAP": "D9E1F2",
    "OPENING": "F5F5F5",
}
FONT_NAME = "Arial"
...
def _inactive_sheet(wb, rows):
    ...
    for i, r in enumerate(rows, 2):
        ...
        vals = [
            ...
            r.model or "",
            r.current_balance,
            r.arrears_outstanding,
            r.reason,
        ]
        for col, v in enumerate(vals, 1):
            # Left-align Name (2), Hub (3), Rider IDs list (4), Reason (10).
            ws.cell(row=i, column=col, value=v).alignment = Alignment(
                horizontal="left" if col in (2, 3, 4, 10) else "center", vertical="center"
            )
...
def build_output_filename(company: str, cycle_start: date, cycle_end: date) -> str:
    return (
        f"payout_{company.lower()}_{cycle_start.strftime('%d%b%Y')}_"
        f"{cycle_end.strftime('%d%b%Y')}.xlsx"
    ).replace(" ", "_")
