...
_RE_DATE_SHIFT = re.compile(r"date\(\s*([^,()]+?)\s*,\s*'([+-]?\d+)\s+days?'\s*\)", re.I)
...
def _func_translations(sql: str) -> str:
    ...
    sql = _RE_DATE_SHIFT.sub(r"to_char((\1)::date + INTERVAL '\2 day', 'YYYY-MM-DD')", sql)
    sql = _RE_DATE_CAST.sub(r"substr(\1, 1, 10)", sql)
    sql = _RE_GROUP_CONCAT.sub(
        lambda m: (
            "string_agg(%s(%s)::text, %s)" % (m.group(1) or "", m.group(2), m.group(3) or "','")  # noqa: UP031
        ),
        sql,
    )
