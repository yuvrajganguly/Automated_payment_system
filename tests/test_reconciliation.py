...
def _seed(conn):
    mid = conn.execute("SELECT model_id FROM ev_models WHERE provider='Raft' AND model_name='Regular'").fetchone()[0]
    conn.execute("INSERT INTO ev_units (ev_id, model_id, status) VALUES ('EV1',?, 'in_use')", (mid,))
    ...
    for d, status, ev in [("2026-03-02","billed",tid),("2026-03-03","billed",tid),
                          ("2026-03-04","billed",tid),("2026-03-05","missed",None),
