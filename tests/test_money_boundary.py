...
def test_rupeeize_covers_the_dashboard_and_ev_rent_keys():
    ...
    out = rupeeize(
        {"owed": 125000, "held": 50000, "days": 7, "rows": [{"collected_current": 250000}]}
    )
    assert out == {
        "owed": 1250.0,
        "held": 500.0,
        "days": 7,
        "rows": [{"collected_current": 2500.0}],
    }
...
def test_cod_clearance_posts_paise_to_the_ledger(db, admin_client):
    ...
    db.execute(
        ...
        "worker_code, amount, source) VALUES ('2026-06-01','2026-06-07','Myntra','C1',?,'C1',50000,'myntra_column')",  # noqa: E501
        (pid,),
    )
    db.commit()
