...
def test_exact_week_advances_seven_days():
    assert allowed_paid_through(
        cur_through="2026-06-14", period_start=None,
        rent_paise=WEEK, weekly_rate=WEEK,
    ) == "2026-06-21"
...
def test_partial_day_floors_down():
    ...
    assert allowed_paid_through(
        cur_through="2026-06-14", period_start=None,
        rent_paise=int(WEEK * 6.9 / 7), weekly_rate=WEEK,
    ) == "2026-06-20"


def test_no_meter_starts_at_period_start():
    assert allowed_paid_through(
        cur_through=None, period_start="2026-06-01",
        rent_paise=WEEK, weekly_rate=WEEK,
    ) == "2026-06-07"
...
def test_no_meter_no_window_returns_none():
    assert allowed_paid_through(
        cur_through=None, period_start=None,
        rent_paise=WEEK, weekly_rate=WEEK,
    ) is None
...
def _seed_assignment(db, ev="EVG1"):
    mid = db.execute(
        "SELECT model_id FROM ev_models WHERE provider='Raft' LIMIT 1"
    ).fetchone()[0]
    db.execute(
        "INSERT INTO ev_units (ev_id, model_id, status) VALUES (?,?,'in_use')",
        (ev, mid))
    pid = db.execute(
        "INSERT INTO person_registry (display_name) VALUES ('G')").lastrowid
    db.execute(
...
def test_backfill_creates_only_missing_rows(db):
    ...
    created = backfill_billed_days(
        db, person_id=pid, event_id=4242,
        day_from="2026-06-15", day_to="2026-06-18")
    db.commit()
    ...
    rows = {r[0]: (r[1], r[2]) for r in db.execute(
        ...
        "WHERE ev_id=? AND day BETWEEN '2026-06-15' AND '2026-06-18'", (ev,))}
...
def test_backfill_without_open_assignment_is_noop(db):
    pid = db.execute(
        "INSERT INTO person_registry (display_name) VALUES ('NoEV')").lastrowid
    db.commit()
