...
def test_attribute_pending_flips_to_billed_up_to_amount(db):
    ...
    applied = attribute_pending(db, person_id=pid, event_id=999, amount=2 * 180.0,
                                day_from="2026-05-01", day_to="2026-05-31")
    db.commit()
    assert applied == round(2 * 180.0, 2)
    billed = db.execute(
        "SELECT COUNT(*) FROM ev_daily_ledger WHERE assigned_person_id=? AND billing_status='billed'",
        (pid,)).fetchone()[0]
    pending = db.execute(
        "SELECT COUNT(*) FROM ev_daily_ledger WHERE assigned_person_id=? "
        "AND billing_status IS NULL AND state='billable'", (pid,)).fetchone()[0]
    assert billed == 2 and pending == 2


def test_attribute_pending_respects_window(db):
    pid = _seed_pending(db, "EVW", ["2026-05-10", "2026-06-10"])
    applied = attribute_pending(db, person_id=pid, event_id=1, amount=10 * 180.0,
                                day_from="2026-05-01", day_to="2026-05-31")
    db.commit()
